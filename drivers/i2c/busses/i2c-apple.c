...
/*
 * I2C bus driver for Apple SoCs (including M1)
 *
...
static void apple_i2c_continue_write(struct apple_i2c *i2c, unsigned max)
{
...
static void apple_i2c_continue_read(struct apple_i2c *i2c)
{
	struct i2c_msg *msg = i2c->msg;
	unsigned num, rxdata;
...
static irqreturn_t apple_i2c_irq(int irq, void *dev_id)
{
	struct apple_i2c *i2c = dev_id;
	struct i2c_msg *msg = i2c->msg;
	unsigned long flags;
	unsigned status;
	...
	if(!msg) {
		...
		if(status & REG_SMSTA_XEN) {
			...
			if(i2c->tx_ptr < msg->len) {
				...
				apple_i2c_continue_write(i2c, MTXFIFO_CHUNK);
...
static void apple_i2c_start_read(struct apple_i2c *i2c)
{
	struct i2c_msg *msg = i2c->msg;
...
static void apple_i2c_start_write(struct apple_i2c *i2c)
{
	struct i2c_msg *msg = i2c->msg;
	...
	apple_i2c_continue_write(i2c, max);

	writel(REG_SMSTA_MTN | REG_SMSTA_XEN | REG_SMSTA_ERROR, i2c->base + REG_IRQMASK);
}

static int apple_i2c_xfer_msg(struct apple_i2c *i2c, struct i2c_msg *msg, int first, int last)
{
	...
	i2c->compl_ptr = 0;
	i2c->tx_ptr = 0;
	i2c->last = 1; /* controller doesn't seem to like leaving a transaction hanging */
	i2c->error = 0;
	...
	if(msg->flags & I2C_M_RD)
		apple_i2c_start_read(i2c);
	else
		apple_i2c_start_write(i2c);
...
static int apple_i2c_xfer(struct i2c_adapter *adap, struct i2c_msg *msgs, int num)
{
	struct apple_i2c *i2c = adap->algo_data;
	int i, ret;
	...
	for(i=0; i<num; i++) {
		ret = apple_i2c_xfer_msg(i2c, &msgs[i], i == 0, i == num - 1);
		if(ret)
			return ret;
	}
...
static int apple_i2c_init_hw(struct apple_i2c *i2c)
{
	...
	i2c->clkdiv = clkdiv;
	dev_err(i2c->dev, "Apple I2C core rev %d, clkdiv %d.\n", i2c->hw_rev, i2c->clkdiv);

	return 0;
}

static u32 apple_i2c_func(struct i2c_adapter *adap)
{
	return I2C_FUNC_I2C | I2C_FUNC_SMBUS_EMUL;
}

static const struct i2c_algorithm apple_i2c_algorithm = {
	.master_xfer	= apple_i2c_xfer,
	.functionality  = apple_i2c_func,
};

static int apple_i2c_probe(struct platform_device *pdev)
{
	struct device_node *np = pdev->dev.of_node;
	struct apple_i2c *i2c;
	struct resource *r;
	int ret = 0, irq;

	i2c = devm_kzalloc(&pdev->dev, sizeof(struct apple_i2c), GFP_KERNEL);
	if(!i2c)
		return -ENOMEM;
	...
	ret = devm_request_irq(&pdev->dev, irq, apple_i2c_irq, 0, dev_name(&pdev->dev), i2c);
	if(ret < 0)
		return ret;

	strlcpy(i2c->adap.name, "Apple I2C adapter", sizeof(i2c->adap.name));
	i2c->adap.owner = THIS_MODULE;
	i2c->adap.algo = &apple_i2c_algorithm;
	i2c->adap.dev.parent = &pdev->dev;
	i2c->adap.dev.of_node = np;
	i2c->adap.algo_data = i2c;

	ret = apple_i2c_init_hw(i2c);
	if(ret)
		return ret;
...
static int apple_i2c_remove(struct platform_device *pdev)
{
	struct apple_i2c *i2c = platform_get_drvdata(pdev);
...
static const struct of_device_id apple_i2c_match[] = {
	{ .compatible = "apple,i2c-v0" },
	{ },
};
MODULE_DEVICE_TABLE(of, apple_i2c_match);

static struct platform_driver apple_i2c_driver = {
	.probe   = apple_i2c_probe,
	.remove  = apple_i2c_remove,
	.driver  = {
		.name  = "apple-i2c",
		.of_match_table = apple_i2c_match,
	},
};
module_platform_driver(apple_i2c_driver);

MODULE_DESCRIPTION("Apple SoC I2C driver");
